<p align="center"><img src="https://raw.githubusercontent.com/titaniumnetwork-dev/Hurricane-static/main/uv.png" height="200">
</p>

<h1 align="center">Hurricane-Node</h1>

<p align="center">The deployable version of Hurricane, a highly sophisticated proxy used for evading internet censorship or accessing websites in a controlled sandbox using the power of service-workers and more!<br><br></p>

## Quick Deployments
[![Deploy to Heroku](https://raw.githubusercontent.com/BinBashBanana/deploy-buttons/master/buttons/remade/heroku.svg)](https://heroku.com/deploy/?template=https://github.com/titaniumnetwork-dev/Hurricane-Node)
[![Run on Replit](https://raw.githubusercontent.com/BinBashBanana/deploy-buttons/master/buttons/remade/replit.svg)](https://replit.com/github/titaniumnetwork-dev/Hurricane-Node)

## Features
- CAPTCHA support along with hCAPTCHA support
- URL encoding settings to further hide activity when using Hurricane
...
Installation of Hurricane is simple. You can find a Tl;DR of the installation and setup process just below. If you are unfamiliar with the "standard" installation process, look a bit farther down for a more comprehensive installation and setup guide.

## Basic Guide

```sh
$ git clone https://github.com/titaniumnetwork-dev/Hurricane-Node --recursive
$ cd Hurricane-Node
$ npm install
$ npm start
```
...
Below will describe a comprehensive guide to install Hurricane on Linux machines.
...
Finally, to start Hurricane, run the following command:

```sh
$ npm start
```

You can then find Hurricane on `http://127.0.0.1:8080`. If you would like to change the port UV will be running on, edit the last line in `index.mjs`. 
...
## Configuration
Configuring Hurricane is very simple. Simple descriptions of each configurable option are provided as a comment in the block below. More detailed documentation can be found just below mentioned block.
...
self.__uv$config = {
    ...
    encodeUrl: Hurricane.codec.xor.encode, // URL Encoding function
    decodeUrl: Hurricane.codec.xor.decode, // Decode URL function
...
| encodeUrl| EncodeUrl is how you want the URL a proxy site's visitors has to be encoded. Options include `Hurricane.codec.base64.encode`, `Hurricane.codec.plain.encode`, or `Hurricane.codec.xor.encode`. It is recommended that you use `xor` or `base64` as it hides the queries your visitors are searching and visiting.
...
Static files is the frontend for Hurricane. A standalone repository for it can be found [here](https://github.com/titaniumnetwork-dev/Hurricane-Static).

## Core Scripts

[Configuration](#configuration) mentions a few scripts that make up Hurricane. To get documentation for what each of the scripts do, check out the [documentation](https://github.com/titaniumnetwork-dev/Hurricane-Core) for them in their standalone repository.

# Main Scripts After Building

The client-hooking & service worker scripts required for UV are located in [Hurricane-scripts](https://github.com/titaniumnetwork-dev/Hurricane-scripts)

- Scripts
    - `uv.sw.js` Service worker gateway
    - `uv.sw-handler.js` - Service worker handler
    - `uv.bundle.js` Webpack compiled Hurricane rewriter
    - `uv.handler.js` Client-side hooking
    - `uv.config.js` Configuration

# Authors

- Caracal.js (Creator of Hurricane)
